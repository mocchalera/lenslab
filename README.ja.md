...
| `NOMINATIM_CONTACT_EMAIL` | 任意 | `/api/geocode` から Nominatim へ送る User-Agent に含める連絡先メールです。 |
...
- 地図ピッカーは `/api/geocode` 経由で OpenStreetMap/Nominatim を利用します。Nominatim の利用制限を守ってください。
