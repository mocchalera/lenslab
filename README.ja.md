...
OpenAI gpt-image-2 または Gemini nano-banana 2（`gemini-3.1-flash-image-preview`）を使い、カメラ・レンズ・ライティング・シーン・服装のコントロールでポートレートを変換するプロ向け光学シミュレーションアプリです。

> このプロジェクトはもともと Google AI Studio でスキャフォールドされ、現在は Vercel プロキシ経由のサーバーアーキテクチャで動作します。

## 特徴

- OpenAI gpt-image-2 と Gemini nano-banana 2（`gemini-3.1-flash-image-preview`）を切り替えられるデュアルプロバイダ構成
...
| `GEMINI_API_KEY` | Gemini 利用時に必須 | `/api/image` が Nano Banana 2（`gemini-3.1-flash-image-preview`）で使う Gemini API キーです。 |
...
Nano Banana 2 は Gemini 3.1 Flash Image Preview モデルです。paid/preview access が必要になる場合があり、preview model のレート制限が適用されます。Google 公式では Gemini 3 Pro Image は Nano Banana Pro（`gemini-3-pro-image-preview`）として別モデルです。
...
- OpenAI Images API (`gpt-image-2`)
- Google Gemini 画像生成（`gemini-3.1-flash-image-preview`）
